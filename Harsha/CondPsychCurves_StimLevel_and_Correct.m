...
%% Shift in P(R) after dopamine reward on easy versus difficult stimuli - separate arguments
    ...
    for blockID = blocks
       ...
       DOPGiven = (action(:,iter)==DopChoice{blockID});
       DOPGiven = reshape(DOPGiven, [nTrials, 1]);
       ...
       for istim = contrasts
          ...
          id=[ false; TStimuli(2:end)==istim  & TBlocks(2:end)==blockID & ...           % current stimulus
              DOPGiven(1:end-1,1) & correct(1:end-1,iter)==1 & IsStimEasy(1:end-1,1)]; % previous action
          PC_next(1,c,blockID,iter)= nanmean(action(id,iter)) ;
          id=[ TStimuli(1:end-1)==istim  & TBlocks(1:end-1)==blockID & ...  
              DOPGiven(2:end,1) & correct(2:end,iter)==1     & IsStimEasy(2:end); false];
          PC_prev(1,c,blockID,iter)= nanmean(action(id,iter)) ;

          %After/Before difficult stim + dopamine
          id=[false; TStimuli(2:end)==istim  & TBlocks(2:end)==blockID & ...           % current stimulus
              DOPGiven(1:end-1,1) & correct(1:end-1,iter)==1 & IsStimDiff(1:end-1)]; % previous action
          PC_next(2,c,blockID,iter)= nanmean(action(id,iter)) ;
          id=[ TStimuli(1:end-1)==istim  & TBlocks(1:end-1)==blockID & ...  
              DOPGiven(2:end,1) & correct(2:end,iter)==1     & IsStimDiff(2:end); false];

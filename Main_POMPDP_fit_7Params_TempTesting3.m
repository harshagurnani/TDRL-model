%% Model details: For STRIATUM DATA
...
% Lapse rate become non-zero upon DA release/reward trials, and return to
% zero after a fixed number of trials without further DA release
%
% Model parameters: 
...
% -     TrialThresh (number of successive trials without DA reward for lapse to become zero)
%-------------------------------------------------------------------------%
%% Fitting Procedure
% 1. Fit sigma, alpha, Da-val, bias, lambdaL, lambdaR, TrialThresh  using all blocks.
...
TrialThresh = [1 2 3 4 5];        %No. of successive trials without DA when lapse becomes zero.

nP = 7;
% Qbias       = -0.8 : 0.05 : 0.8;     

% load in animal data if not already present
AnimalID='ALK022'; % 'SS031_DA' or 'SS040' or 'ALK05' or 'ALK011'
% or 'ALK028' or 'ALK017'
ExpID = '1';
...
% Initialise
Nruns           = length(alpha)*length(DA_val)*length(noiseSTD)*length(Qbias)*length(lambdaL)*length(lambdaR)*length(TrialThresh);
params          = nan(nP,1);
xanswerStore    = nan(Nruns,nP);
FvalStore       = nan(length(alpha),length(DA_val),length(noiseSTD),length(Qbias), length(lambdaL), length(lambdaR), length(TrialThresh) );
...
for datr = 1:length(TrialThresh)
    disp(['DA persistence = ', num2str(TrialThresh(datr)), ' trials'])
...
%    disp(['noiseSTD sample number= ',num2str(kk)])
      ...
      for ii=1:length(alpha)      
         ...
         params(7) = TrialThresh(datr);
         counter = counter + 1;
         
         [Fval, ~,~] = Get_NLL_ModelWithLapseAfterDA(params, Data,  includeContrast, iterN);
...
for model = 1:nModels
    ...
    [data_models(:,:,model), action(:,:,model), correct(:,:,model), ~, ~ ] = RunPOMDP_GS_NLL_withLapseAfterDA(Data3,params);
end
...
NLLData.TrialThresh = TrialThresh;
...
AllThePlots_Bigger_7ParamsStriatum(Data, data_models, action, correct, xanswerMin2, includeContrast, NLLData)
...
beta        = 0.05:0.05:1;           % bias in Q-value
% beta       = -0.8 : 0.05 : 0.8;     
...
% Initialise
Nruns           = length(alpha)*length(DA_val)*length(noiseSTD)*length(beta);
params          = nan(4,1);
xanswerStore    = nan(Nruns,4);
FvalStore       = nan(length(alpha),length(DA_val),length(noiseSTD),length(beta));
...
for bb=1:length(beta)
   ...
   disp(['noiseSTD = ',num2str(kk)])
      ...
      for ii=1:length(alpha)      
         ...
         [Fval] =Fit_POMDP_onMice_IdenticalBelief(params, Data);
...
% beta_new       = xanswerMin(:,4)';        % bias in QValue
% 
% % Initialise

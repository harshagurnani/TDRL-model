...
% -     beta        (Slope of foftmax for generating action probabilities)
%-------------------------------------------------------------------------%
%% Fitting Procedure
% 1. Fit sigma, alpha, Da-val, bias, lambdaL, lambdaR, beta  using all blocks.
...
DA_val      = 0 : 0.5 : 2 ;          % dopamine value 
noiseSTD    = 0.02 : 0.05 : 0.52;    % noise in belief
Qbias       = -0.3 : 0.1 : 0.3;     % bias in Q-value
lambdaL     = 0 :   0.1 : 0.3;       % Probability of lapse to left
lambdaR     = 0 :   0.1 : 0.3;       % Probability of lapse to right
% beta = [1 2 3 4 5];        %No. of successive trials without DA when lapse becomes zero.
beta        = 0.6: 0.3: 1.9;        %No. of successive trials without DA when lapse becomes zero.

nP = 7;
% Qbias       = -0.8 : 0.05 : 0.8;     

% load in animal data if not already present
AnimalID='ALK017 Asymm water'; % 'SS031_DA' or 'SS040' or 'ALK05' or 'ALK011'
% or 'ALK028' or 'ALK017'
ExpID = '1';
...
%%NOT USED AT THE MOMENT FOR THESE FITS
c=1;
for b= blocks 
  for ii= contrast
...
% Initialise
Nruns           = length(alpha)*length(DA_val)*length(noiseSTD)*length(Qbias)*length(lambdaL)*length(lambdaR)*length(beta);
params          = nan(nP,1);
xanswerStore    = nan(Nruns,nP);
FvalStore       = nan(length(alpha),length(DA_val),length(noiseSTD),length(Qbias), length(lambdaL), length(lambdaR), length(beta) );

% Run over parameter values
counter = 0;
% iterN = 3;     %No. of iterations of model to compute probabilities
% Check order of loop is inverse of order of params stored!!!
for datr = 1:length(beta)
    disp(['beta = ', num2str(beta(datr))])
...
%    disp(['noiseSTD sample number= ',num2str(kk)])
      ...
      for ii=1:length(alpha)      
         ...
         [Fval] = Get_NLL_ModelWithLapse_IdenticsalBelief(params, Data,  includeContrast);
         xanswerStore(counter,:) = params;
         FvalStore(ii,jj,kk,bb,lpl,rpr, datr)  = Fval;
      end
      end
   end
end
...
%% NO REFINEMENT OF ALPHA AT THE MOMENT FOR THESE FITS
% %-------------------------------------------------------------------------%
...
%              params(7) = beta_new(kk);
%              
%              counter = counter + 1;
%              [Fval, Fval_l, Fval_r] = Get_NLL_ModelWithLapseAfterDA(params, Data2, includeContrast2, iterN);
...
%-------------------------------------------------------------------------%
%% Step 3: Run top models on all blocks to examine Psychometric curves
%-------------------------------------------------------------------------%
% 
data3     =dataAll;
Data3.data=data3;
...
for model = 1:nModels
    ...
    [data_models(:,:,model), action(:,:,model), correct(:,:,model), ~, ~ ] = RunPOMDP_GS_NLL_withLapse_IdenticalBelief(Data3,params);
end
...
AllThePlots_Bigger_7Params_IdenticalBelief(Data, data_models, action, correct, xanswerMin2, includeContrast, NLLData)
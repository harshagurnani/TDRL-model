...
PlotPC_Mice_Model(data_mice, data_model, action)
   ...
   title(plotMiceRLandErf,...
      sprintf('Mouse data, fitted error function\nand POMDP model'))
   
   % dummy plots for the legend
   h = zeros(4,1);
   h(1) = plot(plotMiceRLandErf,NaN,NaN,'color','b','marker','o',...
      'markersize',10,'markerfacecolor','b','linestyle','none');
   h(2) = plot(plotMiceRLandErf,NaN,NaN,'color','r','marker','o',...
      'markersize',10,'markerfacecolor','r','linestyle','none');
   h(3) = plot(plotMiceRLandErf,NaN,NaN,'color','k','linestyle','-',...
      'linewidth',1.2);
   h(4) = plot(plotMiceRLandErf,NaN,NaN,'color','k','linestyle','--',...
      'linewidth',1.2);
   
   
   legend(h, 'Left','Right','Mouse','Model','Location','southeast');
